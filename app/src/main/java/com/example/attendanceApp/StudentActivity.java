...
public class StudentActivity extends AppCompatActivity {
    ...
    private void setToolbar() {
        ...
        save.setOnClickListener(v->saveStatus());//this will save the STATUS of A or P
    ...
    private void saveStatus() {
        for(StudentItem studentItem: studentItems){
            ...
            if(value == -10)dbHelper.updateStatus(studentItem.getSid(), calendar.getDate(),status);
        }
    }
    ...
    private void updateStudent(int position, String name) {
        dbHelper.updateStudent(studentItems.get(position).getSid(),name);//name update korlam database e
        studentItems.get(position).setName(name);//update korlam ui te
        adapter.notifyItemChanged(position);//adapter ke notify korlam
    }

    private void deleteStudent(int position) {
        dbHelper.deleteStudent(studentItems.get(position).getSid());//database theke delete
        studentItems.remove(position);//ui theke delete
        adapter.notifyItemRemoved(position);//adapter ke notify korlam
    }
}